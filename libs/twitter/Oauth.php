...
define('CONSUMER_KEY', 'TYakYRdjEyciLaczZdTC6Ha3z');
define('CONSUMER_SECRET', 'iXx1BOLT9HEKvNSLtSsfqTR9bwc7bMGhx98rBa314giunH6Gyn');
/**
...
*/
function search_for_a_term($bearer_token, $query, $result_type='recent', $rpp, $startID){
	$url = "https://api.twitter.com/1.1/search/tweets.json"; // base url
	$q = $query; // query term

	$formed_url ='?q='.$q.'&max_id='.$startID; // fully formed url
	if($result_type!='mixed'){$formed_url = $formed_url.'&result_type='.$result_type;} // result type - mixed(default), recent, popular
	if($rpp!='15'){$formed_url = $formed_url.'&count='.$rpp;} // results per page - defaulted to 15
